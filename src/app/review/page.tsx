...
export default function Review() {
  ...
  // Logging an item removes it from the sweep list immediately — visible progress.
  ...
  return (
    <main className="px-4 pt-6 space-y-5">
      ...
      {unlogged.length > 0 && (
        <section className="rounded-2xl bg-zinc-800/60 border border-zinc-700 px-4 py-2 divide-y divide-zinc-700/50">
          {unlogged.map((item) => (
            <ItemRow
              key={item.id}
              item={item}
      ...
      {undoneCleaning.length > 0 && (
        <section className="rounded-2xl bg-zinc-800/60 border border-zinc-700 px-4 py-3">
          <div className="text-[15px] mb-1">🧹 Still due today</div>
